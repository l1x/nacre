...
var _replaceRandom = function _replaceRandom2(s) {
  return s.replace(_randomExp, function(match) {
    ...
    return random(arIndex ? values : +values[0], arIndex ? 0 : +values[1], +values[2] || 0.00001);
  });
};
...
function initOrgTreeConnectors() {
  ...
  function animateDrawIn(allPaths) {
...
function initEpicSelector() {
  ...
  if (!wrapper)
    return;
  const selector3 = wrapper.querySelector(".epic-selector");
  const leftBtn = wrapper.querySelector("#epic-nav-left");
  const rightBtn = wrapper.querySelector("#epic-nav-right");
  if (!selector3 || !leftBtn || !rightBtn)
    return;
  const scrollAmount = 200;
  const updateButtonStates = () => {
    const { scrollLeft, scrollWidth, clientWidth } = selector3;
    leftBtn.disabled = scrollLeft <= 0;
    rightBtn.disabled = scrollLeft + clientWidth >= scrollWidth - 1;
  };
  leftBtn.addEventListener("click", () => {
    selector3.scrollBy({ left: -scrollAmount, behavior: "smooth" });
  });
  rightBtn.addEventListener("click", () => {
    selector3.scrollBy({ left: scrollAmount, behavior: "smooth" });
  });
  selector3.addEventListener("scroll", updateButtonStates);
